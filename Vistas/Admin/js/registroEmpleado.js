...
document.getElementById('btnCancelar').addEventListener('click', function() {    
    if(idCliente != 0){
        fetch(`Controladores/usuario.php?accion=eliminarEmpleado&idEmpleado=${idCliente}`)
        idCliente = 0
    }
...
function submitFormularioInfo(e){
    ...
    fetch('Controladores/usuario.php?accion=registrarEmpleado', {
        method: 'POST',
...
function submitFormularioUsuario(e){
    ...
    datosFormulario.tipoUsuario = localStorage.getItem('tipoUsuario'); localStorage.removeItem('tipoUsuario')
    fetch('Controladores/usuario.php?accion=registrarSuUsuario', {
        method: 'POST',
        body: JSON.stringify(datosFormulario)
    }).then(res => res.json()).then(data => {
        if(data.estado === undefined){
        ...
        if(data.estado === true){
            ...
            e.target.reset()
            setTimeout(() => { window.location.href = 'gestionarUsuarios' }, 3000); return
        }else{
            alertas[5].classList.remove('d-none')
            return
        }
